...
Система состоит из **двух логических частей** (вторая — запланирована):

| Часть | Статус | Назначение |
|-------|--------|------------|
| **VPN-нода** (`xray-node`) | Реализована | Управление одним VPS: inbounds, клиенты, трафик, CLI + HTTP API |
| **Подписочный сервер** | Не реализован | URL на пользователя, выдача конфигов (Happ JSON / base64 links / Clash), агрегация трафика |

Текущий документ описывает **VPN-ноду** полностью и фиксирует требования к подписочному серверу.
...
```
Администратор / подписочный сервер (будущее)
        │  X-API-Key
        ▼
...
Для учёта трафика на нескольких нодах **один и тот же `email`** должен использоваться на всех нодах.
...
- Xray считает трафик по **`email`** на каждой ноде
- `client stats` → `GET /panel/api/clients/traffic/{email}`
- Агрегация по нескольким нодам — задача **подписочного сервера** (будущее)
...
**Доступ к xray-node API с другого сервера** (для будущего подписочного сервера):

```bash
# вариант 1: SSH tunnel
ssh -L 9472:127.0.0.1:9472 root@NODE_IP

# вариант 2: listen 0.0.0.0 + firewall только с IP подписочного сервера
```
...
## 12. Подписочный сервер (план, не реализован)

Для воссоздания полной системы как у коммерческих провайдеров потребуется второй компонент.

### 12.1 Функции

- Один URL на пользователя: `https://sub.example.com/{token}`
- Content negotiation по `User-Agent`:
  - default / curl → base64 список `vless://` ссылок
  - `Happ/1.0` → JSON с observatory, balancers, SMART-профилями
  - `Clash` → YAML
- HTTP-заголовки: `profile-title`, `subscription-userinfo`, `providerid`, …
- Управление клиентами на нодах через **xray-node HTTP API**
- Агрегация `subscription-userinfo` с нескольких нод по `email`

### 12.2 Модель данных пользователя

```
User
  token          # публичный идентификатор подписки
  email          # общий email на всех нодах
  uuid           # общий UUID для VLESS
  traffic_limit
  expiry
  enabled
```
...
```
Subscription Server
  ├── Node 1: https://node1:9472  (X-API-Key)
  ├── Node 2: https://node2:9472
  └── ...
```

На каждой ноде: `POST /clients` с одинаковым `email` / `uuid`.

---

## 13. Чеклист воссоздания системы

### Минимальная рабочая нода
...
### Полная система (с подпиской)

- [ ] Всё из минимальной ноды
- [ ] N нод с одинаковой схемой `email`
- [ ] Подписочный сервер (отдельный VPS)
- [ ] DNS для подписочного домена
- [ ] Генерация Happ JSON / ссылок
- [ ] Cron агрегации трафика
...
| localhost-only API | безопасность; подписочный сервер на том же или через tunnel |
